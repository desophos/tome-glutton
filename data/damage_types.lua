...
newDamageType{
	...
	projector = function(src, x, y, type, dam)
		...
		if target then
			...
			DamageType:get(DamageType.PHYSICAL).projector(src, x, y, DamageType.PHYSICAL, dam.damage)
			
			local satiation = 5 * target.size_category
			local target_life_percent = target.life * 100 / target.max_life
			
			if target_life_percent > dam.devour_power then
			...
			elseif --[[target:canBe("instakill") and ]]not target:attr("self_resurrect") then
				...
				if not target.dead then target:die(src) end
...
newDamageType{
	name = "gorge", type = "GORGE",
	projector = function(src, x, y, type, dam)
		local target = game.level.map(x, y, Map.ACTOR)
		if target then
			local level = 1 -- if we don't know Devour, just use level 1
			if src:knowTalent(src.T_DEVOUR) then
				level = src:getTalentLevel(src:getTalentFromId(src.T_DEVOUR))
			end
			DamageType = require "engine.DamageType"
			-- do our extra physical damage
			DamageType:get(DamageType.PHYSICAL).projector(src, x, y, DamageType.PHYSICAL, dam)
			if not src:isTalentActive(src.T_VORACITY) then -- hack so that we don't Devour enemies twice
				-- try to Devour this target
				src:forceUseTalent(src.T_DEVOUR, {force_target=target, force_level=level, ignore_energy=true, ignore_ressources=true, ignore_cd=true, no_confirm=true})
			end
		end
	end,
}
